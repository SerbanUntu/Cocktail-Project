...
const cocktailDialog = document.getElementById('cocktail-dialog');
const dialogButton = document.getElementById('close-dialog');
const dialogContent = document.getElementById('dialog-content');
const cocktailFormButton = document.getElementById('search-cocktail-button');
...
cocktailForm.addEventListener("submit", (e) => {
    ...
    const searchText = formData.get('search-cocktail');
    ...
    getCocktails(searchText, searchType).then(cocktails => {
        ...
        function generateGrid(cocktails) {
            cocktails.forEach(cocktail => {
                // Container of the cocktail card definition
                const cocktailCardContainer = document.createElement('div');
                ...
                switch (cocktail.alcoholic) {
                    case 0:
                        alcoholicIconContainer.style.opacity = '0';
                        break;
                    case 1:
                        alcoholicIconContainer.style.backgroundColor = 'gray';
                        break;
                }
                ...
                cocktailCard.onclick = function () {
                    ...
                    const alcoholicText = document.getElementById('dialog-alcoholic-text');
                    switch (cocktail.alcoholic) {
                        case 0:
                            alcoholicText.innerText = 'No';
                            break;
                        case 1:
                            alcoholicText.innerText = 'Optional';
                            break;
                        default:
                            alcoholicText.innerText = 'Yes';
                            break;
                    }
                    ...
                    const instructionsText = document.getElementById('dialog-instructions-text');
...
function getCocktails(searchText, searchType) {
        ...
        return yield fetch(url)
            ...
            res.drinks.forEach((drink) => {
                ...
                if (searchType !== 'by-ingredient') {
                    ...
                    for (let i = 1; i <= 15; i++) {
                        let ingredientProperty = 'strIngredient' + i.toString();
                        let measureProperty = 'strMeasure' + i.toString();
                        let value = '';
...
function ensureDrinksFetched(len) {
    let time = Date.now();
    return new Promise(waitForDrinks);
    function waitForDrinks(resolve, reject) {
...
//Navbar indicator
const buttonCOCKTAILS = document.getElementById('navbar-cocktails-button');
